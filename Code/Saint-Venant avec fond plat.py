...
arrowgap = 0.2
arrowx, arrowy = np.meshgrid(wbhllsw0.getX()[::int(N/L*arrowgap)],  np.linspace(-1,0.5, 30))
arrowy[:,::2] += 0.5*(arrowy[0,0]-arrowy[1,0])
quiverU, quiverZero = np.meshgrid( np.roll(U0, N//4)[::int(N/L*arrowgap)],  np.linspace(-1,0.5, 30)*0)
quiverU *= arrowy<np.meshgrid( np.roll(S0, N//4)[::int(N/L*arrowgap)]-0.01,  np.linspace(-1,0.5, 30)*0)[0]
plt.figure("initial value",figsize=(10,3))
plt.title("t = 0")
plt.fill_between(wbhllsw0.getX() , np.roll(S0, N//4), -1)
plt.plot(wbhllsw0.getX(), np.roll(S0, N//4), "b")
plt.plot(wbhllsw0.getX(), wbhllsw0.getX()*0-1, "k")
plt.quiver(arrowx, arrowy, quiverU, quiverZero, scale = 7, minlength = 0.001,label = "vitesse")
plt.xlabel("x")
plt.legend()
plt.show()
plt.pause(0.1)
...
plt.clf()
plt.title("Norme L2")
plt.plot(np.linspace(0,T,len(norm)), norm2, "k")
plt.xlabel("t")
plt.legend()
plt.show()
