...
amplitudes = [0.4]
labels = ["sous-optimale", "optimale", "sur-optimale"]
...
for amplitude in amplitudes:
    def b_func(X):
        return amplitude*(1 + np.sin(2*300*np.pi*X/L))/2
...
for i, amplitude in enumerate(amplitudes):
    plt.plot((-2*(F(sws[i].getSurface()) - F(S0))/(T*(1e-15 + F(sws[i].getSurface())+F(S0))))[:arg], label = " amplitude = "+str(amplitude)+" ("+labels[i]+")")
...
plt.semilogy(F(wbhllsw.getSurface())[:arg],  label = "T = "+str(T)+" (fond oscillant)")
plt.xlabel("$k$")
