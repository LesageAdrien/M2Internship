...
def b_func(X):
    return 0.2*(1 + np.sin(100*np.pi*X/L))/2

"""Construction d'une solution initiale unidirectionnelle"""
def zeta_init(X):
    return np.exp(-20*(X-0.9*L/2)**2)*2
...
S0[wbhllsw0.getX()>(-L/4)] *=0
U0[wbhllsw0.getX()>(-L/4)] *=0
...
plt.clf()
plt.title("Norme L2")
plt.plot(np.linspace(0,T,len(norm)), norm2, "k--", label = "fond plat")
plt.plot(np.linspace(0,T,len(norm)), norm, label = "fond oscillant")
plt.xlabel("t")
plt.legend()
plt.show()
